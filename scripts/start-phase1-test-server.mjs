...
if (!Number.isInteger(port) || port < 1024 || port > 65535) throw new Error("A valid non-privileged test port is required.");
...
const build = spawn(process.execPath, [nextBinary, "build", "--webpack"], {
...
async function stop(signal = "SIGTERM") {
  ...
  await rm(temporaryApplication, { recursive: true, force: true });
  process.stdout.write("Phase 1 test server cleanup complete.\n");
}

for (const signal of ["SIGINT", "SIGTERM", "SIGHUP"]) {
  process.on(signal, async () => {
    await stop(signal === "SIGINT" ? "SIGINT" : "SIGTERM");
    process.exit(0);
  });
}
