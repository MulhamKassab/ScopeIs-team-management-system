...
try {
  await withDisposableTestDatabase("phase10_collaboration", async ({ databaseUrl, env }) => {
    ...
    const result = await runChild(process.execPath, [join(repositoryRoot, "node_modules", "vitest", "vitest.mjs"), "run", "test/integration/phase10-collaboration-service.test.ts", ...process.argv.slice(2)], { cwd: repositoryRoot, env, timeoutMs: 300_000 });
