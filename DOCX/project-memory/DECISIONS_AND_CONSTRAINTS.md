...
- Super Admin account administration is a narrow, Super Admin-only journey: accounts are created with their workforce profile atomically, sign-in can be enabled for an existing workforce record, and passwords can be reset. Existing passwords are one-way hashes and can never be viewed or recovered; a Super Admin sets a temporary password instead. Reset and change revoke sessions. See `SUPER_ADMIN_ACCOUNT_MANAGEMENT_DECISIONS.md`.
