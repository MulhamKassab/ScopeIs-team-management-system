...
const roleCapabilities: Record<SystemRole, readonly Capability[]> = {
  ...
  ADMIN: ["dashboard", "employees", "skills", "clients", "projects", "locations", "schedule", "map", "leave", "coverage", "replacements", "notifications", "reports", "profile"].map((module) => `module:${module}:view` as Capability).concat("foundation:scope_probe:view"),
  EMPLOYEE: ["dashboard", "skills", "schedule", "leave", "profile", "notifications", "requests"].map((module) => `module:${module}:view` as Capability),
};
