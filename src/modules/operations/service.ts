...
export class OperationalService {
  ...
  /**
   * Phase 10: an edit preserves the superseded content as a revision row in the same transaction, so a
   * shared note can never be silently overwritten. Only the original author may edit.
   */
  async updateNote(actor: AuthenticatedActor, input: unknown) {
    const parsed = parseOperational(noteUpdateSchema, input); const note = await operationalRepository.note(db, parsed.noteId);
    if (!note || note.archivedAt) throw new OperationalDomainError("NOT_FOUND");
    const target = note.clientId ? { type: "CLIENT" as const, id: note.clientId } : note.projectId ? { type: "PROJECT" as const, id: note.projectId } : { type: "LOCATION" as const, id: note.locationId! };
    await this.requireTarget(db, actor, target);
    if (note.authorUserId !== actor.id) throw new OperationalDomainError("FORBIDDEN");
    return db.transaction(async (tx) => {
      await operationalRepository.lockNote(tx, note.id);
      const current = await operationalRepository.note(tx, note.id);
      if (!current || current.archivedAt) throw new OperationalDomainError("NOT_FOUND");
      if (current.version !== parsed.expectedVersion) throw new OperationalDomainError("STALE_VERSION");
      await operationalRepository.createNoteRevision(tx, { noteId: current.id, version: current.version, content: current.content, editedByUserId: actor.id });
      const row = await operationalRepository.updateNote(tx, current.id, parsed.expectedVersion, parsed.content);
      if (!row) throw new OperationalDomainError("STALE_VERSION");
      await this.audit(tx, actor, "operational_note.updated", "operational_note", row.id, { targetType: target.type, targetId: target.id, version: row.version, previousVersion: current.version });
      // The revision write is audited separately so a preserved copy is independently traceable.
      await this.audit(tx, actor, "operational_note.revision_created", "operational_note", row.id, { targetType: target.type, targetId: target.id, previousVersion: current.version, contentLength: current.content.length });
      return row;
    });
  }
