...
type CreateEmployeeField = "displayName" | "employeeCode" | "workEmail" | "workPhone" | "professionalSummary";
...
function validationErrors(input: unknown): CreateEmployeeFormState | null {
  ...
  return {
    fieldErrors: {
      displayName: fieldErrors.displayName?.[0], employeeCode: fieldErrors.employeeCode?.[0],
...
export const createEmployeeAction: CreateEmployeeFormAction = async (_previousState, formData) => {
  ...
  const input = {
    displayName: formText(formData, "displayName"), employeeCode: formText(formData, "employeeCode"),
  ...
  try {
    ...
    if (error instanceof EmployeeDomainError) {
      if (error.code === "CONFLICT") return { fieldErrors: { employeeCode: "That employee code is already in use." } };
