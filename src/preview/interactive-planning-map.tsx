...
import { LoadingSkeleton } from "@/preview/motion-system";
...
export function InteractivePlanningMap({ persona }: { persona: PreviewPersona }) {
  ...
  const handleMapReady = useCallback((instance: LeafletMap) => { setMap(instance); setMapReady(true); }, []);
  ...
  return <>
    ...
    <section className={`planning-map-shell ${mapReady ? "is-ready" : "is-loading"}`} aria-label="Interactive published planning map" aria-busy={!mapReady}>
      {!mapReady ? <LoadingSkeleton variant="map" label="Loading the interactive planning map" /> : null}
      <MapContainer center={defaultCenter} zoom={defaultZoom} scrollWheelZoom className="planning-map" aria-label="Published schedule planning map">
        <MapLifecycle onMapReady={handleMapReady} />
